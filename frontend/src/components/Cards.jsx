...
function Cards({ image }) {
  ...
  return (
    ...
    >
      ...
      <div
        className={`absolute inset-0 bg-gradient-to-b from-transparent via-transparent to-cyan-900/20 transition-opacity duration-500 z-10 
      ...
      <div className="absolute inset-0 z-20 flex flex-col justify-between p-4 pointer-events-none">
        {/* Top: Tech Status */}
        <div
          className={`flex justify-between items-start transition-all duration-300 transform
           ${
             isSelected
        ...
        >
          <div
            className={`px-2 py-1 backdrop-blur-md border rounded text-[10px] font-mono uppercase tracking-widest
             ${
               isSelected
          ...
          <div
            className={`w-2 h-2 rounded-full shadow-[0_0_10px_#22d3ee] 
             ${
               isSelected
                 ? "bg-white animate-none shadow-[0_0_15px_white]"
        ...
        >
          <div
            className={`h-[1px] w-full mb-3 transition-opacity duration-300
             ${
               isSelected
                 ? "bg-cyan-400 opacity-100"
                 : "bg-gradient-to-r from-transparent via-cyan-500/50 to-transparent opacity-0 group-hover:opacity-100"
             }`}
          ></div>

          <p
            className={`text-center font-mono text-xs tracking-[0.2em] uppercase transition-opacity duration-300
             ${
               isSelected
                 ? "text-cyan-300 opacity-100 font-bold"
      ...
      <div
        className={`absolute top-2 left-2 w-4 h-4 border-l-2 border-t-2 transition-colors duration-300 z-30
        ${
          isSelected
            ? "border-cyan-400"
      ...
      <div
        className={`absolute bottom-2 right-2 w-4 h-4 border-r-2 border-b-2 transition-colors duration-300 z-30
        ${
          isSelected
            ? "border-cyan-400"
