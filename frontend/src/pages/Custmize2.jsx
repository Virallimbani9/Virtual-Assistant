            ...
            <div className="relative z-10 w-full max-w-lg">
                ...
                <div className="relative bg-[#0a0a0a]/80 backdrop-blur-xl rounded-2xl border border-white/10 shadow-2xl p-8 md:p-12 flex flex-col items-center">
                    ...
                    >
                        ...
                        <span className="text-[10px] font-mono uppercase tracking-widest opacity-70 group-hover/back:opacity-100">Back</span>
                    </button>
                    ...
                    <div className="mb-10 relative group mt-4">
                        <div className={`absolute inset-0 bg-cyan-500 blur-xl rounded-full transition-all duration-500 ${isLoading ? 'opacity-70 scale-125' : (assistantName ? 'opacity-40' : 'opacity-10')}`}></div>
                        ...
                        <div className="relative w-24 h-24 bg-black border border-cyan-500/30 rounded-full flex items-center justify-center shadow-[inset_0_0_20px_rgba(6,182,212,0.2)]">
                            <FaRobot className={`w-10 h-10 transition-all duration-500 ${assistantName || isLoading ? 'text-cyan-300 drop-shadow-[0_0_10px_rgba(6,182,212,0.8)]' : 'text-gray-600'}`} />

                            {/* Orbit Rings */}
                            <div className={`absolute inset-0 rounded-full border border-cyan-500/20 border-t-transparent ${isLoading ? 'animate-[spin_1s_linear_infinite]' : 'animate-[spin_4s_linear_infinite]'}`}></div>
                            <div className={`absolute inset-2 rounded-full border border-violet-500/20 border-b-transparent ${isLoading ? 'animate-[spin_0.7s_linear_infinite_reverse]' : 'animate-[spin_3s_linear_infinite_reverse]'}`}></div>
                        </div>
                    </div>
                    ...
                    <div className="text-center mb-10">
                        <div className="inline-flex items-center gap-2 px-3 py-1 rounded bg-cyan-950/30 border border-cyan-500/20 mb-3">
                            <span className={`w-1.5 h-1.5 rounded-full bg-cyan-400 ${isLoading ? 'animate-ping' : 'animate-pulse'}`}></span>
                    ...
                    {/* --- INPUT FIELD --- */}
                    <div className={`w-full relative mb-8 group transition-opacity duration-300 ${isLoading ? 'opacity-50 pointer-events-none' : 'opacity-100'}`}>
                        <input
                            type="text"
                    ...
                    <div className="h-16 w-full flex justify-center items-center">
                        {assistantName && (
                            ...
                            >
                                <div className={`absolute inset-0 w-full h-full bg-gradient-to-r from-violet-600 via-cyan-600 to-violet-600 transition-opacity duration-300 bg-[length:200%_auto] 
                                    ${isLoading ? 'opacity-100 animate-gradient' : 'opacity-90 group-hover:opacity-100'}`}>
                                </div>
                    ...
                    <div className="mt-6 flex justify-between w-full text-[10px] font-mono text-gray-600 uppercase">
                        <span>System: {isLoading ? 'UPLOADING...' : (assistantName ? 'Ready' : 'Waiting')}</span>
                    </div>
