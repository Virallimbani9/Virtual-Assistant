  ...
  return (
    <div className="relative w-full min-h-screen bg-[#030712] text-white flex flex-col items-center overflow-x-hidden font-sans">
      ...
      <div className="relative z-10 w-full max-w-7xl px-4 py-12 md:py-16 flex flex-col items-center">
        ...
        {/* --- BACK BUTTON (UPDATED) --- */}
        {/* Changed 'top-0' to 'top-10' (lowered position) */}
        {/* Changed text size to 'text-sm' and icon to 'w-5 h-5' (larger) */}
        <button 
            onClick={() => navigate("/home")} 
            className="absolute top-10 left-6 md:left-0 text-gray-400 hover:text-cyan-400 transition-colors duration-300 flex items-center gap-3 group/back z-50 cursor-pointer"
        >
            <FaArrowLeft className="w-5 h-5 group-hover/back:-translate-x-1 transition-transform duration-300" />
            <span className="text-sm font-mono uppercase tracking-widest font-bold opacity-80 group-hover/back:opacity-100">Back Home</span>
        </button>

        {/* Header Section */}
        <div className="text-center mb-12 md:mb-16 mt-12 relative w-full px-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-10 perspective-1000 place-items-center w-full">
          ...
          >
            ...
            >
              ...
              <div
                className={`absolute inset-0 rounded-full border-t border-cyan-400/50 animate-spin ${
                  isUploadSelected
                    ? "opacity-100"
            ...
            <div className="text-center z-10">
              <h3
                className={`font-bold text-lg transition-colors tracking-wide ${
                  isUploadSelected
                    ? "text-cyan-300"
            ...
            {/* Tech Corners */}
            <div
              className={`absolute top-2 left-2 w-3 h-3 border-l border-t transition-colors ${
                isUploadSelected
                  ? "border-cyan-400"
            ...
            ></div>
            <div
              className={`absolute bottom-2 right-2 w-3 h-3 border-r border-b transition-colors ${
                isUploadSelected
                  ? "border-cyan-400"
                  : "border-gray-600 group-hover:border-cyan-400"
              }`}
            ...
            <div
              className={`absolute inset-0 bg-[linear-gradient(45deg,transparent_25%,rgba(6,182,212,0.05)_50%,transparent_75%,transparent_100%)] bg-[length:250%_250%,100%_100%] bg-no-repeat transition-opacity duration-700 pointer-events-none group-hover:animate-[shine_2s_infinite]
              ${
                isUploadSelected
                  ? "opacity-100 animate-[shine_2s_infinite]"
                  : "opacity-0 group-hover:opacity-100"
              }`}
            ></div>
          </label>
        ...
        <div className="mt-16 mb-8 w-full flex justify-center">
          {selectedImage && (
            <button
              className="group relative px-10 py-4 bg-transparent overflow-hidden rounded-lg transition-all duration-300 hover:shadow-[0_0_30px_rgba(139,92,246,0.5)] active:scale-95 cursor-pointer"
              onClick={() => {
                navigate("/custmize2");
              }}
            >
        ...
        <div className="mt-4 flex items-center gap-2 text-gray-600 font-mono text-[10px] md:text-xs">
          ...
          ></div>
          {selectedImage
            ? "PILOT SELECTION LOCKED"
            : "SYSTEM ONLINE // WAITING FOR SELECTION"}
        </div>
      </div>
    </div>
  );
